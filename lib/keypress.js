...
const isShiftKey = code => {
  let shiftKeys = ['[a', '[b', '[c', '[d', '[e', '[2$', '[3$', '[5$', '[6$', '[7$', '[8$', '[Z'];
  return shiftKeys.includes(code);
};

const isCtrlKey = code => {
  let ctrlKeys = ['Oa', 'Ob', 'Oc', 'Od', 'Oe', '[2^', '[3^', '[5^', '[6^', '[7^', '[8^'];
  return ctrlKeys.includes(code);
};
...
keypress.action = (buf, key, customActions) => {
  ...
  if (typeof buf === 'string' && buf.charCodeAt(0) === 160 && key.name === void 0) {
    key.name = 'space';

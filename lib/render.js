...
module.exports = prompt => {
  return async(input, context = {}) => {
    ...
    let sep = context.separator || await prompt.separator();
    let message = context.message || await prompt.message();

    // "defalut hint" is only used on Boolean prompts
    let hint = prompt.default ? prompt.styles.muted(prompt.default) : null;

    // ? Select your favorite colors > (y/N) Input
    // ^             ^               ^   ^     ^
    // prefix     message    separator  hint input
    let promptLine = [prefix, message, sep, hint].filter(Boolean).join(' ');
    prompt.state.prompt = promptLine;

    let header = context.header || await prompt.header();
    let value = prompt.cast ? prompt.cast(input) : input;

import osSignals from './shims/osSignals.js';
...
export default function createUtils(platformShims) {
  const { colors, exit, isWindows, onOsSignal } = platformShims;
  ...
  function resolve(context, value, ...rest) {
  ...
  function scrollDown(choices = []) {
    return [...choices.slice(1), choices[0]];
  }

  function scrollUp(choices = []) {
  ...
  function width(stream, fallback = 80) {
    let columns = (stream && stream.columns) ? stream.columns : fallback;
    if (stream && typeof stream.getWindowSize === 'function') {
      columns = stream.getWindowSize()[0];
    }

    return isWindows() ? columns - 1 : columns;
  }
  ...
  function mixin(target, b) {
    if (!isObject(target)) return b;
    if (!isObject(b)) return target;
    for (let key of Object.keys(b)) {
      let desc = Object.getOwnPropertyDescriptor(b, key);
      if (Object.prototype.hasOwnProperty.call(desc, 'value')) {
        if (Object.prototype.hasOwnProperty.call(target, key) && isObject(desc.value)) {
  ...
  function mixinEmitter(obj, emitter) {
  ...
  function defineExport(obj, key, fn) {
    let custom;

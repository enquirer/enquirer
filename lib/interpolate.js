...
const compile = (input, options = {}) => {
  ...
  return (state = {}) => {
    ...
    for (let token of tokens) {
      ...
      if (token.type === 'template') {
        let choice = choices.find(ch => ch.name === token.key);
        let val = first([choice.typed, choice.value, value]);

        if (state.answered) {
          let result = format(state.values[token.key], choice, state);
        ...
        if (val !== value) {
          ...
          if (state.missing.has(token.key) || state.invalid.has(token.key)) {

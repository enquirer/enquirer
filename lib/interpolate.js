  ...
  while (i < input.length - 1) {
    ...
    if ((value === '$' || value === '#' || value === '{') && peek() === '{') {
      ...
      while ((ch = next())) {
        ...
        while (ch === '\\') esc += next();
        if (esc.length && esc.length % 2 === 0) {
          ch += esc.slice(esc.length / 2);
        } else if (esc) {
          ch += next();
        }
...
module.exports = async prompt => {
  ...
  return async(state = {}, submitted = false) => {
    ...
    let validate = async(value, state, token, item, index) => {
      let error = await isValid(value, state, token, item, index);
      if (error === false) {
    ...
    for (let token of tabstops) {
      ...
      if (token.type === 'template') {
        ...
        if (submitted) {
          let error = await validate(state.values[key], state, token, item, index);
          ...
          let res = await result(state.values[key], state, token, item, index);
        ...
        let before = value;
        value = await format(value, state, token, item, index);
...
function createFn(prop, prompt, options, fallback) {
  return (value, state, token, item, index) => {
    if (typeof item.field[prop] === 'function') {
      return item.field[prop].call(prompt, value, state, token, item, index);
    }

...
export default function createState(platformShims) {
  const { stdout } = platformShims;
  const { define, width } = createUtils(platformShims);
  ...
  class State {
    constructor(prompt) {
      ...
      this.prompt = '';
      this.buffer = '';
      this.width = options.stdout ? width(options.stdout) : stdout.columns;

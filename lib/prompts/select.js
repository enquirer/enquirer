...
class SelectPrompt extends ArrayPrompt {
  constructor(options) {
    ...
    this.state.hint = first([this.state.hint, 'Use arrow keys, <return> to submit.']);
  ...
  async reset() {
    await super.reset();
    let initial = this.findIndex(this.initial || 0);
    let { index, autofocus } = this.options;
    this.index = first('number', [autofocus, index, initial]);
  }
  ...
  footer() {
    if (!this.answered && !this.options.footer && this.choices.length > this.limit) {

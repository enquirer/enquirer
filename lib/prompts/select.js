...
class SelectPrompt extends ArrayPrompt {
  ...
  async renderChoice(choice, i) {
    ...
    if (hint && !colors.hasColor(hint)) hint = this.styles.muted(hint);
  ...
  async renderChoices() {
    ...
    let choices = await Promise.all(this.visible.map(async(ch, i) => {
      return await this.renderChoice(ch, i);
    }));
  ...
  format() {
    ...
    if (!state.submitted) return styles.muted(state.hint);
    if (Array.isArray(selected)) {
      return selected.map(choice => styles.primary(choice.name)).join(', ');
    }
    return styles.primary(selected.name);
  }

...
class SelectPrompt extends ArrayPrompt {
  constructor(options = {}) {
    ...
    if (options.hint == null) {
      this.state.set('hint', '(Use arrow keys, <return> to submit)');
    }

    if (options.footer == null) {
      this.state.set('footer', '(Use <fn>+<up/down> to reveal more choices)');
    }
  ...
  pointer(choice, i) {
    return this.element('pointer', this.state, this.index === i ? 'on' : 'off');
  }
  ...
  footer() {
    if (this.options.footer || (!this.answered && this.choices.length > this.limit)) {
      return super.footer();
    }
  }

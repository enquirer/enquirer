...
class HorizontalList extends Prompt {
  constructor(options = {}) {
    ...
    this.cursor = 0;
    this.color('active', colors.underline);
  }
  ...
  renderChoice(choice, i) {
    return this.cursor === i ? this.colors.active(choice.message) : choice.message;
  }
  ...
  get sep() {
    return this._sep || this.options.sep || ` ${this.colors.hint(this.symbols.middot)} `;
  }

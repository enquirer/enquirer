import createArrayPrompt from '../types/array.js';

export default function createSurvey(platformShims) {
  const { env } = platformShims;
  const ArrayPrompt = createArrayPrompt(platformShims);

  class Survey extends ArrayPrompt {
    constructor(options = {}) {
      super(options);
      this.emptyError = options.emptyError || 'No items were selected';
      this.term = env.get('TERM_PROGRAM');

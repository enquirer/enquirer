...
class TogglePrompt extends BooleanPrompt {
  ...
  render() {
    this.clear();
    let values = [
      this.value ? this.disabled : this.colors.active.underline(this.disabled),
      this.value ? this.colors.active.underline(this.enabled) : this.enabled
    ];
    let message = this.renderMessage(values.join(this.colors.hint(' / ')));
    this.write(message);
  }
}

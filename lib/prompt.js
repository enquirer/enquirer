...
class Prompt extends Events {
  constructor(options = {}) {
    ...
    this.setOption('validate', () => true);
    this.setOption('format', () => this.value);
    this.setOption('result', val => val);
    this.setOption('stdout', process.stdout);
    this.setOption('stdin', process.stdin);
    this.setOption('initial');
  }

  setOption(key, fallback) {
  ...
  initializeOptions(options) {
    for (let key of Object.keys(options)) {
      let opt = options[key];
      if (typeof opt === 'function' && typeof this[key] === 'function') {
        this[key] = opt.bind(this);
  ...
  element(name, ...rest) {
    let { options, state, symbols } = this;
    let value = options[name] || symbols[name] || state[name];
    return utils.resolve(state, value, state, ...rest);
  }

  prefix() {
    let { state, styles, symbols } = this;
    let element = this.element('prefix') || symbols;
    if (utils.isObject(element)) element = element[state.status] || element.pending;
    let style = styles[state.status];
  ...
  separator() {
    let element = this.element('separator');
    let ele = element[this.state.status] || element;
  ...
  pointer(choice, i) {
    let element = this.element('pointer', choice, i);
    if (element) {
  ...
  indicator(choice, i) {
    let element = this.element('indicator', choice, i);
    if (element) {
      let styles = this.styles;
  ...
  hint() {
    ...
    if (!this.isValue(input) && status === 'pending') {
      return this.styles.muted(this.element('hint'));
    }
  }

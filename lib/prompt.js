...
class Prompt extends Events {
  ...
  async separator() {
    ...
    let value = this.state.separator || element[this.state.status] || element;
    let ele = await utils.resolve(this.state, value, this.state);
  ...
  async message() {
    let element = await this.element('message');
    if (!utils.hasColor(element)) {
      return this.styles.strong(element);
    }
    return element;
  }

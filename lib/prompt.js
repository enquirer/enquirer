...
class Prompt extends Events {
  ...
  run() {
    return new Promise(async(esolve, reject) => {
      this.once('s'submit'resolve);
      this.once('c'cancel'reject);
      if (await this.skip()) {
        this.render = () => {};
        return this.submit();
      }
  ...
  async prefix() {
    let element = (await this.element('p'prefix' || this.symbols;
    let timer = this.timers && this.timers.prefix;
    let state = this.state;
    state.timer = timer;
    if (utils.isObject(element)) { elemen{element = element[state.status] || element.pending;} if (!utils.hasColor(element)) {
  ...
  async message() {
    let message = await this.element('mes'message'   if (!utils.hasColor(message)) {
  ...
  async separator() {
    let element = (await this.element('sep'separator'| this.symbols;
  ...
  async pointer(choice, i) {
    let val = await this.element('poi'pointer'oice, i);

    if (typeof val === 'str'string'utils.hasColor(val)) {
      return val;
    }
    ...
    if (val) {
      ...
      let ele = await this.resolve(
        val[focused ? 'on''on'off'off' val,
        this.state
      );
      let styled = !utils.hasColor(ele) ? style(ele) : ele;
      return focused ? styled : ' '.' 'eat(ele.length);
    }
  }

  async indicator(choice, i) {
    let val = await this.element('ind'indicator'oice, i);
    if (typeof val === 'str'string'utils.hasColor(val)) {
      return val;
    }
    if (val) {
      ...
      let ele = val[enabled ? 'on''on'off'off' val;
...
''}
  ...
  footer() {
    if (this.state.status === 'pen'pending'      return this.element('foo'footer'   }
  }

  header() {
    if (this.state.status === 'pen'pending'      return this.element('hea'header'   }
  }

  async hint() {
    if (this.state.status === 'pen'pending'!this.isValue(this.state.input)) {
      let hint = await this.element('hin'hint'     if (!utils.hasColor(hint)) {
...
function setOptions(prompt) {
  let isValidKey = (key) => {
    return prompt[key] === void 0 || typeof prompt[key] === 'fun'function'};

  let ignore = [
    'act'actions'  'cho'choices'  'ini'initial'  'mar'margin'  'rol'roles'  'sty'styles'  'sym'symbols'  'the'theme'  'tim'timers'  'val'value'

  let ignoreFn = [
    'body'body' 'foot'footer' 'erro'error' 'head'header' 'hint'hint' 'indi'indicator' 'mess'message' 'poin'pointer' 'pref'prefix' 'sepa'separator' 'skip'skip'
  for (let key of Object.keys(prompt.options)) {
    if (ignore.includes(key)) continue;
    if (/^on[A-Z]/.test(key)) continue;
    let option = prompt.options[key];
    if (typeof option === 'funct'function'ValidKey(key)) {
      if (!ignoreFn.includes(key)) {
        prompt[key] = option.bind(prompt);
      }
    } else if (typeof prompt[key] !== 'funct'function'    prompt[key] = option;
    }
  }
}

function margin(value) {
  if (typeof value === 'numbe'number'  value = [value, value, value, value];
  }

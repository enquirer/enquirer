...
import createKeypress from './keypress.js';

export default function createPrompt(platformShims) {
  /**
   * Base class for creating a new Prompt.
   * @param {Object} `options` Question object.
   */

  const { colors, env, stdout, stdin } = platformShims;
  const State = createState(platformShims);
  const ansi = createAnsi(platformShims);
  const theme = createTheme(platformShims);
  const utils = createUtils(platformShims);
  const keypress = createKeypress(platformShims);

  class Prompt extends platformShims.EventEmitter {
    constructor(options = {}) {
      super();
      ...
      this.term = this.options.term || env.get('TERM_PROGRAM');
    ...
    get height() {
      return this.options.rows || stdout.rows;
    }
    get width() {

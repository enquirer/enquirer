...
class ArrayPrompt extends Prompt {
  constructor(options) {
    ...
    this.longest = 0;
    this.num = '';
    this.choiceValueSelector = options.choiceValueSelector
      ? typeof options.choiceValueSelector === 'function'
        ? options.choiceValueSelector
        : (choice) => choice[options.choiceValueSelector]
      : (choice) => choice['name'];
  }
  ...
  async submit() {
    ...
    this.value = multi ? value.map(this.choiceValueSelector) : this.choiceValueSelector(value);
    return super.submit();
  }

...
export default function createArrayPrompt(platformShims) {
  const { colors } = platformShims;
  const Prompt = createPrompt(platformShims);
  const utils = createUtils(platformShims);
  const roles = createRoles(platformShims);
  ...
  class ArrayPrompt extends Prompt {
    ...
    async reset() {
      ...
      if (utils.isObject(initial)) initial = Object.keys(initial);
    ...
    number(n) {
      ...
      let number = num => {
        ...
        if (this.visible.indexOf(choice) === -1) {
          let choices = utils.reorder(this.choices);
    ...
    home() {
      this.choices = utils.reorder(this.choices);
      this.index = 0;
      return this.render();
    }
    ...
    scrollUp(i = 0) {
      this.choices = utils.scrollUp(this.choices);
      this.index = i;
      if (this.isDisabled()) {
        return this.up();
      }
    ...
    scrollDown(i = this.visible.length - 1) {
      this.choices = utils.scrollDown(this.choices);
      this.index = i;
      if (this.isDisabled()) {
        return this.down();
      }
    ...
    swap(pos) {
      utils.swap(this.choices, this.index, pos);
    }
    ...
    find(value, prop) {
      if (utils.isObject(value)) return prop ? value[prop] : value;
      let isChoice = (ele, i) => [ele.name, i].includes(value);
      let fn = typeof value === 'function' ? value : isChoice;

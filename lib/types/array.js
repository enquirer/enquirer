...
const { choice, choices, increment, decrement, reorder } = require('../utils');
const scrollDown = choices => [...choices.slice(1), choices[0]];
const scrollUp = choices => [choices.pop(), ...choices];
...
class ArrayPrompt extends Prompt {
  ...
  number(n) {
    ...
    this.index = this.visible.indexOf(choice);
    this.toggle();
    return this.render();
  }
  ...
  g() {
    if (!this.choices.some(choice => !!choice.parent)) return this.a();
  ...
  filter(value, prop) {
    ...
    let result = this.choices.filter(filter);
    if (prop) {
  ...
  async submit(val) {
    ...
    this.value = this.multiple === true
      ? choice.map(choice => choice.name)
      : choice ? choice.name : void 0;

    if (Array.isArray(this.value)) {
      this.values = {};
      choice.forEach(ch => (this.values[ch.name] = ch.value));
    }

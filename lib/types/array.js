...
const { reorder, scrollUp, scrollDown, isObject } = require('../utils');
...
class ArrayPrompt extends Prompt {
  ...
  a() {
    ...
    this.choices.forEach(choice => choice.enabled = !enabled && !choice.disabled);
    return this.render();
  }
  ...
  focus(choice, enabled) {
    ...
    choice.enabled = enabled && !choice.disabled;
  }

  toggle(choice = this.focused) {
    choice = this.find(choice);
    choice.enabled = !choice.enabled && !choice.disabled;
  ...
  isEnabled(choice = this.focused) {
    ...
    return choice.enabled && !choice.disabled;
  }
  ...
  get enabled() {
    return this.filter(choice => !choice.disabled && choice.enabled);
  }
